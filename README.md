...
> **`terraform destroy` 時の注意**: バージョンレベル不変性が有効な Storage Account は `AccountProtectedFromDeletion` により **Unlocked 状態でも `terraform destroy` 単体では削除できません**。CD ワークフローの Destroy ジョブには事前クリーンアップステップが組み込まれており、`az storage account delete` で自動削除した後に state から除外します。手動で destroy を実行する場合は以下を先に実行してください。
>
> ```bash
> az storage account delete --name stlawarchive<region> --resource-group rg-management-<region> --yes
...
| **LAW アーカイブ Storage Account** | `AccountProtectedFromDeletion` — バージョンレベル不変性が有効なため削除保護される | **CD では自動処理済**（Pre-Destroy Cleanup ステップ）。手動 destroy の場合は `az storage account delete` → `terraform state rm`（[詳細手順](#不変ポリシーworm)） |
