...
├── policy-exemptions.tf          # ポリシー免除（exemptions/ YAML から展開）
...
| 特定リソースをポリシーから免除したい | `exemptions/` に YAML 追加（[ポリシー免除](#ポリシー免除exemptions)参照） |
...
そのようなリソースには **ポリシー免除** を `exemptions/` ディレクトリの YAML ファイルで管理します。

```
exemptions/
  terraform-state-sa.yaml   # Terraform state backend SA の免除
```
...
免除の適用には `terraform_state_storage_account_id` 変数でリソース ID を指定する必要があります。  
空文字（デフォルト）の場合は免除リソースを作成しません。
