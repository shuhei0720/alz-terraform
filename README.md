...
> **`terraform destroy` 時の注意**: バージョンレベル不変性が有効な Storage Account は `AccountProtectedFromDeletion` により **Unlocked 状態でも `terraform destroy` 単体では削除できません**。CD ワークフローの Destroy ジョブには事前クリーンアップステップが組み込まれており、パブリックアクセス有効化 → Blob 不変性ポリシー削除 → Blob 削除 → Storage Account 削除 → state 除外を自動実行します。手動で destroy を実行する場合は以下を先に実行してください。
>
> ```bash
> # 1. パブリックアクセスを有効化（データプレーン操作に必要）
> az storage account update -n stlawarchive<region> -g rg-management-<region> --public-network-access Enabled
> # 2. アカウントキーを取得
> KEY=$(az storage account keys list -n stlawarchive<region> -g rg-management-<region> --query "[0].value" -o tsv)
> # 3. Blob 不変性ポリシーを削除し Blob を削除（全コンテナ・全バージョン）
> for C in $(az storage container list --account-name stlawarchive<region> --account-key "$KEY" -o tsv --query "[].name"); do
>   az storage blob list --account-name stlawarchive<region> -c "$C" --account-key "$KEY" --include v --query "[].{name:name,versionId:versionId}" -o tsv | while IFS=$'\t' read -r NAME VER; do
>     ARGS=(--account-name stlawarchive<region> -c "$C" -b "$NAME" --account-key "$KEY")
>     [[ -n "$VER" ]] && ARGS+=(--version-id "$VER")
>     az storage blob immutability-policy delete "${ARGS[@]}" 2>/dev/null || true
>     az storage blob delete "${ARGS[@]}" 2>/dev/null || true
>   done
> done
> # 4. Storage Account を削除し state から除外
> az storage account delete -n stlawarchive<region> -g rg-management-<region> --yes
> terraform state rm 'azurerm_storage_account.law_archive[0]'
> ```
>
> Locked 状態の場合はさらにポリシー保持期間が経過するまで削除不可のため、Azure Portal でポリシーを先に解除してください。
