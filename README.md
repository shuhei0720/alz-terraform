...
本構成は複数リージョンに Hub VNet をデプロイし、`active_hub_key` 変数 1 つで全 Spoke の接続先を一括切替できる DR（ディザスタリカバリ）設計になっています。

#### 設計思想

- **通常時**: `active_hub_key = "primary"`（プライマリ Hub 経由で通信）
- **DR 時**: `active_hub_key = "secondary"` に変更して `terraform apply` → 全 Spoke がセカンダリ Hub に切替

セカンダリ Hub は常に稼働状態（Firewall, DNS Resolver, Gateway がデプロイ済み）のため、切替後即座に通信可能です。

#### `active_hub_key` で切り替わるリソース
...
```bash
# 1. terraform.tfvars の active_hub_key を変更
active_hub_key = "secondary"   # primary → secondary
