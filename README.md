...
│   ├── architecture_definitions/          # 管理グループの一覧と、各 MG に適用するポリシーセット名の対応表
│   │   └── alz_with_amba.*.json           # ALZ + AMBA 統合定義
│   ├── archetype_definitions/             # 各管理グループのポリシーセット定義（追加・除外のカスタマイズ）
...
└── .github/
    ...
    ├── workflows/
    │   ├── ci.yaml                        # PR 時: Security Scan + fmt + validate + plan
...
| サブスクリプション | 管理グループ | 用途 |
|:---|:---|:---|
| Management | Platform > Management | LAW, Sentinel, UAMI × 2, DCR |
...
管理グループを作るには、以下の 3 つの作業が必要です:
1. **管理グループリソースの作成**（Azure 上に MG を作成する）
2. **ポリシーの継承設定**（新 MG をポリシーエンジンに認識させ、親 MG のポリシーを継承させる）
3. **ポリシーのカスタマイズ**（継承したポリシーに対して、追加・除外が必要な場合に備える）

#### Step 1: management-groups.tf にリソース追加（MG リソースの作成）
...
#### Step 2: architecture_definition.json にエントリ追加（ポリシー継承の設定）

このファイルは「どの管理グループに、どのポリシーセット（アーキタイプ）を適用するか」の対応表です。
エントリを追加することで、ポリシーエンジンが新 MG を認識し、親 MG からのポリシー継承が有効になります。

ファイル: `lib/architecture_definitions/alz_with_amba.alz_architecture_definition.json`
...
#### Step 3: archetype override YAML 作成（ポリシーのカスタマイズ）

「アーキタイプ」とは、管理グループ 1 つ分のポリシー構成（どのポリシーを割り当てるか）をまとめた定義です。
この YAML で、ベースとなるポリシーセットに対して追加・除外をカスタマイズできます。

```yaml
# lib/archetype_definitions/confidential_custom.alz_archetype_override.yaml
base_archetype: corp          # ベースにする既存ポリシーセット（corp, online 等）
name: confidential_custom     # Step 2 の archetypes で指定した名前と一致させる

policy_assignments_to_add: []     # ベースに追加するポリシー割り当て
policy_assignments_to_remove: []  # ベースから除外するポリシー割り当て
policy_definitions_to_add: []     # ベースに追加するポリシー定義
policy_definitions_to_remove: []  # ベースから除外するポリシー定義
```

`base_archetype` に既存のポリシーセットを指定すると、そのポリシー構成を継承した上で追加・除外ができます。
カスタマイズが不要な場合でも、空の YAML ファイルは必要です（ポリシーエンジンがファイルの存在を前提としているため）。
...
> **Hub VNet のアドレス空間について**: `terraform.tfvars.example` では `/22`（1,024 IP）を使用しています。各サブネットは第三オクテットごとに配置（`10.0.0.x` = GW/Bastion、`10.0.1.x` = Firewall、`10.0.2.x` = DNS Resolver）しており、サブネットの用途を IP アドレスから視認しやすくしています。サブネットプレフィックスは用途に応じて `/26`〜`/28` を割り当てています。将来の拡張が必要な場合は `/20` 等に拡大してください。
...
| 2（通常） | YAML に `hub_key` が指定されている | `virtual_network.hub_key` の値 |
...
| カテゴリ | 種別 | 許可内容 |
|:---|:---|:---|
| **インフラストラクチャ** | Network | DNS (53), KMS (1688), Azure Monitor Agent, Machine Config (Arc), RHEL RHUI パッチ |
| **Microsoft 365 + AKS** | Network | Exchange, SharePoint, Teams, AKS API Server / NTP |
| **プラットフォーム** | Network | Azure Portal, Entra ID, ARM, Front Door |
| **OS 更新** | Application | Windows Update (FQDN Tag), Ubuntu パッケージ |
...
├── RC: AllowInfrastructure (DNS, KMS, AMA, RHUI, Azure Portal ...)
├── RC: AllowM365andAKS
├── RC: AllowPlatformServices
└── RC: AllowCloudServices (Windows Update, AKS FQDN Tag)
...
firewall_rules:
  network_rules:
    - name: AllowSQL
      protocols: [TCP]
      source_addresses: ["10.200.0.0/26"]
      destination_addresses: ["10.200.0.64/26"]
      destination_ports: ["1433"]

    - name: AllowRedis
      protocols: [TCP]
      source_addresses: ["10.200.0.0/26"]
      destination_addresses: ["10.200.0.128/26"]
...
そのため、本リポジトリではBastionの録画は有効化しておりません。
...
Forwarding Ruleset は Hub VNet にリンクされ、Hub VNet 内の DNS クエリに転送ルールが適用されます。Spoke VNet はインバウンドエンドポイント経由で Hub に DNS クエリを送信するため、Spoke のクエリにも同じ転送ルールが適用されます。
...
55 ゾーンは主要な Azure サービス（Storage, SQL, Key Vault, Cosmos DB, Event Hub, Service Bus, Web Apps 等）をカバーしています。
...
| やりたいこと | 方法 |
|:---|:---|
| ゾーンを追加 | [variables.tf](variables.tf) の `private_dns_zones` の default リストに追加 |
| 特定のゾーンだけ使う | `terraform.tfvars` で `private_dns_zones` を上書き指定（デフォルト全置換） |
| DNS 自体を無効化 | `terraform.tfvars` で `private_dns_enabled = false` |

> **リージョン固有ゾーン**: 以下の 4 ゾーンは `japaneast` 固有です。`primary_location` を変更する場合は [variables.tf](variables.tf) で置換してください。
...
ゾーン一覧は [variables.tf](variables.tf) の `private_dns_zones` を参照してください。
...
policy_default_values = {
  # 全ポリシーの LAW 送信先を統一
  log_analytics_workspace_id              = jsonencode({ value = azurerm_log_analytics_workspace.main.id })
  # AMA エージェントの認証 ID
  ama_user_assigned_managed_identity_id   = jsonencode({ value = azurerm_user_assigned_identity.ama.id })
  ama_user_assigned_managed_identity_name = jsonencode({ value = azurerm_user_assigned_identity.ama.name })
  # AMBA アラートの通知先
  amba_alz_action_group_email             = jsonencode({ value = var.amba_alert_email })
}
```
...
サブスクリプションを一度作成した後、`terraform destroy` で削除して再作成しようとするとエイリアス競合エラーが発生します。
（`terraform destroy` はリソースの削除と同時に Terraform State からも削除されるため、Terraform はサブスクリプションの存在を認識できなくなります）
...
hub_virtual_networks = {
  primary = {
    ...
    dns_resolver_inbound_subnet_prefix  = "10.0.2.0/28"
...
> **前提**: 以下のコマンドは Azure CLI（`az`）を使用します。`az login` で適切なサブスクリプションにログイン済みであることを確認してください。
...
| **Required checks** | `Security Scan`, `Validate & Plan`（CI ワークフローのジョブ名） |

> **ポイント**: Required checks に CI ジョブ名 `Security Scan` と `Validate & Plan` を追加することで、セキュリティスキャンまたは plan が失敗した PR はマージできなくなります。
...
```
PR 作成/更新
  ┌─ Security Scan ジョブ（並列・Azure 認証不要）
  │  → Trivy IaC スキャン
  │  → SARIF → GitHub Code Scanning（Security タブ + PR diff にインライン表示）
  │  → HIGH/CRITICAL → PR コメント投稿
  │  → HIGH/CRITICAL 検出時はジョブ失敗（マージブロック）
  │
  └─ Validate & Plan ジョブ（並列）
...
[Trivy](https://trivy.dev/)（Aqua Security）による IaC セキュリティスキャンを実行します。
Azure 認証・Terraform init が不要なため、plan ジョブと**並列で即実行**されます。

| 設定 | 値 | 説明 |
|:---|:---|:---|
| **ツール** | Trivy（tfsec 後継） | Terraform の設定ミス・CIS ベンチマーク違反を検出 |
| **SARIF** | GitHub Code Scanning にアップロード | Security タブで管理 + PR diff にインライン表示（MEDIUM 以上） |
| **PR コメント** | HIGH/CRITICAL のみ | レビュアーが即確認できる |
| **Gate** | HIGH/CRITICAL で失敗 | `.trivyignore` で許容済みの検出を除外可能 |
| **バージョン管理** | Dependabot（github-actions） | `aquasecurity/trivy-action` の更新を週次で自動検知 → PR 作成 |

`.trivyignore` にアーキテクチャ上許容する検出 ID を記載することで、既知の許容事項を管理できます：

```
# .trivyignore
AVD-AZU-XXXX   # 理由: ALZ 設計上 ～～ のため許容
```

> **参考**: [Aqua Vulnerability Database](https://avd.aquasec.com/) で検出 ID の詳細を確認できます。
...
| **permissions** | `id-token: write`, `pull-requests: write`, `security-events: write` | OIDC 認証 + PR コメント投稿 + Code Scanning |
| **plan 出力** | PR コメントに貼付 | 60,000 文字を超える場合は先頭/末尾各 30,000 文字に切り出し（最大 65,536 文字） |
...
| Private DNS Zone | `Conflict`, `409` | 10s | 60s | 55 ゾーン同時作成の API 衝突 |
...
`terraform.tfvars` に `active_hub_key = "secondary"` を追加し、PR を作成してマージします。
CD パイプラインが自動で apply し、全 Spoke VNet のピアリング先がセカンダリ Hub に切り替わります。
切り戻しは `active_hub_key` の行を削除（またはコメントアウト）するだけです。
