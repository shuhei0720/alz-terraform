...
├── policy-exemptions.tf          # ポリシー免除（グローバル + サブスク YAML から展開）
...
| 特定リソースをポリシーから免除したい | `lib/policy_exemptions/` または `subscriptions/<name>.yaml` の `policy_exemptions`（[ポリシー免除](#ポリシー免除exemptions)参照） |
