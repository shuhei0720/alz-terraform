...
├── policy-exemptions.tf          # ポリシー免除（lib/policy_exemptions/ YAML から展開）
...
| 特定リソースをポリシーから免除したい | `lib/policy_exemptions/` に YAML 追加（[ポリシー免除](#ポリシー免除exemptions)参照） |
...
| 10 | `ddos_protection_plan_id` | DDoS Protection Plan ID（割り当て除外済みだがダミー値が必要） | Enable-DDoS-VNET |
...
本リポジトリでは **全 29 ポリシーを `Default`（強制）モードに切り替え** ています。  
...
なお `Enable-DDoS-VNET` 割り当て自体は `connectivity` / `landing_zones` アーキタイプから除外済みです。

#### ポリシー免除（Exemptions）

ガードレール強制化により、一部のプラットフォーム基盤リソースがポリシーに違反する場合があります。  
そのようなリソースには **ポリシー免除** を `lib/policy_exemptions/` の YAML ファイルで管理します。

Azure Policy の免除は**スコープ**（管理グループ / サブスクリプション / リソースグループ / リソース）を指定して、そのスコープに対するポリシー適用を除外する仕組みです。
...
免除を追加するには、YAML ファイルを作成または編集します:

```yaml
exemptions:
  - name: exempt-my-resource             # 一意の免除名（Azure リソース名）
    policy_assignment: Enforce-GR-Storage0  # 免除対象のポリシー割り当て名
    management_group_suffix: platform     # ポリシー割り当て元 MG（${root_id}-{suffix}）
    scope: ${terraform_state_storage_account_id}  # 免除スコープ（${変数名} で変数参照可）
    category: Waiver                      # Waiver（恒久）or Mitigated（代替措置あり）
    display_name: "説明的な名前"
    description: "免除理由の詳細"
```
...
| `policy_assignment` | 免除対象のポリシー割り当て名 |
| `management_group_suffix` | ポリシーが割り当てられている管理グループのサフィックス |
| `scope` | 免除のスコープ（リソース ID）。`${変数名}` で `variables.tf` の変数を参照可能 |
| `category` | `Waiver`（恒久的免除）または `Mitigated`（代替措置により免除） |

scope に `${変数名}` を使用した場合、その変数が空文字なら免除は作成されません。
...
> **`terraform destroy` 時の注意**: バージョンレベル不変性が有効な Storage Account は `AccountProtectedFromDeletion` により **Unlocked 状態でも `terraform destroy` 単体では削除できません**。CD ワークフローの Destroy ジョブには Pre-Destroy Cleanup ステップが組み込まれており自動処理されます。手動で destroy する場合は以下の順序でクリーンアップしてください。
>
> 1. パブリックアクセスを有効化（データプレーン操作に必要）
> 2. アカウントキーを取得
> 3. 全 Blob の不変性ポリシーを削除し Blob を削除（**az CLI は `--version-id` 非対応のため、過去バージョンには REST API + SharedKey 認証が必要**）
> 4. コンテナを管理プレーンで削除（`az storage container-rm delete` — データプレーンでは不変性有効コンテナの削除不可）
> 5. Storage Account を削除し `terraform state rm` で state から除外
>
> 具体的な実装は [cd.yaml の Pre-Destroy Cleanup ステップ](.github/workflows/cd.yaml) を参照してください。
...
| **AMA 用 UAMI** | `RequestDisallowedByPolicy` — ALZ の `DenyAction-DeleteUAMIAMA` ポリシーが削除を拒否 | ポリシーアサインメントが先に削除されるため **destroy を再実行すれば自動解消** |
