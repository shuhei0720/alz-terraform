...
`terraform force-unlock <LOCK_ID>` で手動解除できます。ロック ID はエラーメッセージに表示されます。

### Q: `terraform destroy` で一部リソースが削除できない場合は？

Azure 管理グループはソフトデリート（論理削除）されるため、`destroy` 後も Azure 内部で保持されます。
再作成時に「already exists」エラーが出た場合は、Azure Portal の「管理グループ」から完全削除してください。
`azapi` リソースで `Missing Resource Identity After Read` エラーが出る場合は、Azure 側でリソースが既に削除されています。`terraform state rm` で state から除外するか、空の state を `terraform state push -force` で上書きしてください。
