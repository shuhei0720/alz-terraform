...
| `management_group_suffix` | ✅ | ポリシーが割り当てられている MG のサフィックス（`${root_id}-{suffix}` の `{suffix}` 部分。例: `platform`, `landingzones`） |
| `scope` | ✅ | 免除スコープのリソース ID。`${変数名}` で `variables.tf` の変数を参照可能 |
...
1. `policy-exemptions.tf` が `lib/policy_exemptions/*.yaml` を自動検出・読み込み
