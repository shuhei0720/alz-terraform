...
> **`terraform destroy` 時の注意**: バージョンレベル不変性が有効な Storage Account は `AccountProtectedFromDeletion` により **Unlocked 状態でも `terraform destroy` で削除できません**。destroy 前に以下のコマンドで手動削除してください。
>
> ```bash
> # 1. コンテナ内の全 Blob を削除（不変ポリシーが Unlocked かつ保持期間外の場合）
> az storage blob delete-batch \
>   --account-name stlawarchive<region> \
>   --source am-law-archive \
>   --auth-mode login
>
> # 2. Storage Account を手動削除
> az storage account delete \
>   --name stlawarchive<region> \
>   --resource-group rg-management-<region> \
>   --yes
>
> # 3. Terraform state から除外（Azure 側で既に削除済みの場合）
> terraform state rm 'azurerm_storage_account.law_archive[0]'
> ```
>
> Locked 状態の場合はさらにポリシー保持期間が経過するまで Blob 削除不可のため、Azure Portal でポリシーを先に解除してください。
...
| **LAW アーカイブ Storage Account** | `AccountProtectedFromDeletion` — バージョンレベル不変性が有効なため削除保護される | Blob を全削除 → `az storage account delete` で手動削除 → `terraform state rm` で state から除外（[詳細手順](#不変ポリシーworm)） |
